...
(
    ...
    INDEX `fk_Casa_Propietario_idx`
(
    `Propietario_CodigoPropietario` ASC
) ,
    INDEX `fk_Casa_Departamento1_idx`
(
    `Departamento_CodigoDepartamento` ASC
) ,
    INDEX `fk_Casa_País1_idx`
(
    `País_CodigoPais` ASC
) ,
    INDEX `fk_Casa_Municipio1_idx`
(
    `Municipio_CodigoMunicipio` ASC
) ,
    CONSTRAINT `fk_Casa_Propietario`
    FOREIGN KEY
(
    `Propietario_CodigoPropietario`
)
    REFERENCES `app-kodigo`.`Propietario`
(
    `CodigoPropietario`
)
    ON DELETE NO ACTION
    ...
    FOREIGN KEY
(
    `Departamento_CodigoDepartamento`
)
    REFERENCES `app-kodigo`.`Departamento`
(
    `CodigoDepartamento`
)
    ON DELETE NO ACTION
...
(
    ...
    FOREIGN KEY
(
    `Municipio_CodigoMunicipio`
)
    REFERENCES `app-kodigo`.`Municipio`
(
    `CodigoMunicipio`
)
    ON DELETE NO ACTION
...
(
    ...
    INDEX `fk_Departamento_País1_idx`
(
    `País_CodigoPais` ASC
) ,
...
(
    ...
    INDEX `fk_Factura_Servicio1_idx`
(
    `Servicio_CodigoServicio` ASC
) ,
    CONSTRAINT `fk_Factura_Servicio1`
    FOREIGN KEY
(
    `Servicio_CodigoServicio`
)
    REFERENCES `app-kodigo`.`Servicio`
(
    `CodigoServicio`
)
    ON DELETE NO ACTION
...
(
    ...
    INDEX `fk_Persona_Casa1_idx`
(
    `Casa_CodigoCasa` ASC
) ,
    UNIQUE INDEX `Correo_UNIQUE`
(
    `Correo` ASC
) ,
...
(
    ...
    INDEX `fk_Municipio_Departamento1_idx`
(
    `Departamento_CodigoDepartamento` ASC
) ,
...
(
    ...
    UNIQUE INDEX `NombrePais_UNIQUE`
(
    `NombrePais` ASC
) )
    ENGINE = InnoDB;
...
(
    ...
    INDEX `fk_Servicio_Casa1_idx`
(
    `Casa_CodigoCasa` ASC
) ,
    INDEX `fk_Servicio_Proveedor1_idx`
(
    `Proveedor_CodigoProveedor` ASC
) ,
...
(
    ...
    FOREIGN KEY
(
    `Proveedor_CodigoProveedor`
)
    REFERENCES `app-kodigo`.`Proveedor`
(
    `CodigoProveedor`
)
    ON DELETE NO ACTION
